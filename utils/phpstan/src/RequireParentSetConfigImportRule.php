...
{
    private const string ERROR_MESSAGE = 'Config file "%s" must be imported in parent set config "%s"';
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if (! str_contains($parentSetFilePath, '/config/sets/')) {
            return [];
        }

        // is this a parent set config? it must have a sibling directory of the same name
        $nestedSetDirectory = substr($parentSetFilePath, 0, -strlen('.php'));
        ...
        foreach ($nestedSetFilePaths as $nestedSetFilePath) {
            ...
            if (in_array($nestedSetFileName, $importedFileNames, true)) {
                continue;
            }

            $ruleErrors[] = RuleErrorBuilder::message(
                sprintf(self::ERROR_MESSAGE, $nestedSetFileName, basename($parentSetFilePath))
            )
                ->identifier('rector.parentSetConfigImport')
                ->build();
        }
    ...
    private function resolveImportedFileNames(Return_ $return): array
    {
        ...
        foreach ($methodCalls as $methodCall) {
            ...
            foreach ($strings as $string) {
                if (! str_ends_with($string->value, '.php')) {
                    continue;
                }
