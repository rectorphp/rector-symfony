...
CODE_SAMPLE
    ...
    public function refactor(Node $node): ?Node
    {
        foreach ($node->stmts as $classStmtKey => $classStmt) {
            ...
            foreach ($classStmt->stmts as $classMethodStmtKey => $methodStmt) {
                ...
                if ($classConstraintNew instanceof New_ && $this->refactorClassConstraint(
                    $node,
                    $classConstraintNew
                )) {
                ...
                if ($classMethodAndConstraint instanceof ClassMethodAndConstraint && $this->refactorGetterConstraint(
                    $node,
                    $classMethodAndConstraint
                )) {
                    unset($classStmt->stmts[$classMethodStmtKey]);
                    $hasChanged = true;
                    continue;
                ...
                if ($propertyAndConstraint instanceof PropertyAndConstraint && $this->refactorPropertyConstraint(
                    $node,
                    $propertyAndConstraint
                )) {
                    unset($classStmt->stmts[$classMethodStmtKey]);
                    $hasChanged = true;
                }
    ...
    private function refactorGetterConstraint(
        Class_ $class,
        ClassMethodAndConstraint $classMethodAndConstraint
    ): bool {
        ...
        foreach ($classMethodAndConstraint->getPossibleMethodNames() as $possibleMethodName) {
            ...
            $classMethod->attrGroups[] = $this->constraintAttributeGroupFactory->create(
                $constraintClass,
                $constraintNew
            );
