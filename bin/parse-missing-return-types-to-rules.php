...
final class MissingReturnTypeParser
{
    ...
    public function run(): void
    {
        ...
        foreach ($returnTypeChanges as $returnTypeChange) {
            $configurationContent .= sprintf(
                "('%s', '%s', %s)" . PHP_EOL,
                $returnTypeChange->getClass(),
                $returnTypeChange->getMethod(),
                $returnTypeChange->getReturnType(),
            );
        }
    ...
    private function resolveDiffFileToReturnTypeChanges(string $fileDiffPath): array
    {
        ...
        foreach ($fileDiffs as $fileDiff) {
            ...
            foreach ($matches as $match) {
                ...
                $returnTypeChanges[] = new ReturnTypeChange(
                    $className,
                    $methodNameMatch['method_name'],
                    $newTypeMatch['return_type'],
                );
            }
        }
